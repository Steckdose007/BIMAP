...
def main():
    ...
    dataset = dataset.rename_column('edit_prompt', 'labels')
    dataset = dataset.rename_column('edited_image', 'image')
    # To check if the changes have been applied
    print(dataset.column_names)
    unique_values = set(dataset['labels'])
    print(unique_values)

    # Create a dictionary that maps each unique value to a unique integer
    value_to_label = {value: i for i, value in enumerate(unique_values)}
    print(value_to_label)
    import time
    time.sleep(15)
    # Use the map function to change the value column to a label column
    dataset = dataset.map(lambda example: {'labels': value_to_label[example['labels']]})
    unique_values = set(dataset['labels'])
    print(unique_values)
    ...
    if isinstance(data_args.train_val_split, float) and data_args.train_val_split > 0.0:
        dataset = dataset.train_test_split(data_args.train_val_split)
        dataset["validation"] = dataset["test"]
        del dataset["test"]
        print(dataset)
        import time
        time.sleep(15)
        # dataset["original_image"] = split["train"]
        # dataset["edited_image"] = split["test"]
